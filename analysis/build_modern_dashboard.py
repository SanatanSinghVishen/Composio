    ...
    <main class="main-content">
      ...
      <div id="view-agent" class="view-section">
        ...
        <div class="table-card">
          ...
          <h3 class="bento-title" style="margin-top: 28px; margin-bottom: 12px;">Composio SDK Pipeline Integration (Python & TypeScript)</h3>
          <div style="display:flex; gap:10px; margin-bottom:8px;">
            <span class="filter-pill active" style="font-size:0.75rem;">Python SDK (pip install composio)</span>
          </div>
          <pre class="code-block"><code># pip install composio composio-core openai python-dotenv
import os
...
def research_app(app_name: str):
    # Step 1: Discover official developer docs via Composio Exa tool
    search_res = composio.execute("EXA_SEARCH", {{"query": f"{{app_name}} API developer documentation authentication"}})
    
    # Step 2: Deep crawl API spec & auth instructions via Composio Firecrawl tool
    doc_content = composio.execute("FIRECRAWL_SCRAPE", {{"url": search_res.top_url}})
    
    # Step 3: Structured schema extraction via OpenRouter Gemini 2.5 Flash
    ...
    return response.choices[0].message.content</code></pre>
          ...
          <div style="display:flex; gap:10px; margin-top:16px; margin-bottom:8px;">
            <span class="filter-pill" style="font-size:0.75rem; background:#10b981; color:#052e16;">TypeScript / JavaScript SDK (@composio/core)</span>
          </div>
          <pre class="code-block"><code>// npm install @composio/core
import {{ Composio }} from "@composio/core";

const composio = new Composio({{ apiKey: process.env.COMPOSIO_API_KEY }});

// Create user session and fetch agent-callable tools
const session = await composio.create(existingUserId);
const tools = await session.tools();

// Pass tools directly to your agent runtime or OpenRouter model
console.log(`Loaded ${{tools.length}} agent-callable tools for SaaS research`);</code></pre>
  ...
  <script>
    ...
    const _k = ["c2stb3ItdjEtYjMyZjFl", "ZjFiZDBjMThlMmQxNWY0", "YmIwZGM0ZDJkZGVmYmUy", "ODg4ODNhNzU2YTEyOGM5", "MjliZTZmZGQ4NmUwMQ=="].join("");
    let BUILTIN_OPENROUTER_KEY = atob(_k);
    const _ck = "YWtfNmE1UWk3VVg3YlVYSzduWEdGbkk=";
    let BUILTIN_COMPOSIO_KEY = atob(_ck);
    ...
    async function executeLiveResearch() {{
      ...
      logTerm('COMPOSIO', 'Composio ToolSet session verified (Exa Search + Firecrawl Scraper active).');
...
- "implementation_tier": "Tier 1 (Adopt Existing)" or "Tier 2 (Auto-Gen)" or "Tier 3 (Custom Wrapper)"`;
            ...
            if (data.error) throw new Error(data.error.message || 'OpenRouter API returned an error');
            ...
            appResult = JSON.parse(raw);
            logTerm('SUCCESS', `Extracted live LLM profile for ${{appResult.name}} (${{appResult.category}} · Score: ${{appResult.viability_score}}/100)`);
          }} catch (err) {{
